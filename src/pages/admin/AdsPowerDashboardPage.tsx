...
import { HiOutlinePlus, HiChevronDown, HiChevronUp, HiDatabase, HiOutlinePencil, HiTrash, HiInformationCircle, HiServer } from "react-icons/hi";
                    ...
                    {profiles.map((profile: AdsPowerProfile) => (
                      <React.Fragment key={profile.user_id}>
                        <Table.Row className="hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                          ...
                          <Table.Cell>
                            {profile.last_open_time ? new Date(profile.last_open_time * 1000).toLocaleString() : "Nigdy"}
                          </Table.Cell>
                          <Table.Cell>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                  <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                                    ...
                                    <div className="text-sm text-gray-500 dark:text-gray-400">
                                      <p>System: {profile.fingerprint_config?.browser_kernel_config?.type || "chrome"}</p>
                                      <p>Wersja: {profile.fingerprint_config?.browser_kernel_config?.version || "N/A"}</p>
                                      <p>Rozdzielczość: {profile.fingerprint_config?.screen_resolution || "1920x1080"}</p>
      ...
      <section className="bg-gray-50 dark:bg-gray-900 py-3 sm:py-5 mt-8">
        <div className="mx-auto max-w-screen-2xl">
          <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            {isLoadingBrightData ? renderBrightDataLoading() : errorBrightData ? renderBrightDataError() : (
              <div className="overflow-x-auto">
                      ...
                      {brightDataZones.map((zone: BrightDataProxy) => (
                        <React.Fragment key={zone.zone}>
                          <Table.Row className="hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                            ...
                            <Table.Cell>
                              <Badge color={zone.status === "active" ? "success" : zone.status === "active_details_unavailable" ? "purple" : "warning"}>
                                {zone.status || "Nieznany"}
                              </Badge>
                            </Table.Cell>
                            <Table.Cell>{zone.created_at ? new Date(zone.created_at).toLocaleDateString() : "N/A"}</Table.Cell>
                            <Table.Cell>
                              <div className="flex items-center space-x-2">
                                <Button
                                  size="xs"
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    <div className="md:col-span-full mt-3 pt-3 border-t dark:border-gray-600">
                                      <p className="font-semibold text-gray-900 dark:text-white mb-1">Informacje Dostępowe (Super Proxy):</p>
                                      <p>Host: <strong>brd.superproxy.io</strong></p>
                                      <p>
                                        Port: 
                                    ...
                                    <div className="md:col-span-full mt-3 pt-3 border-t dark:border-gray-600">
                                      ...
                                      <p>Start: {zone.plan_details?.start_date ? new Date(zone.plan_details.start_date).toLocaleString() : "N/A"}</p>
                                      <p>Typ: {zone.plan_details?.type || "N/A"}</p>
                                      <p>Typ VIP: {zone.plan_details?.vips_type || "N/A"}</p>
                                      <p>Produkt: {zone.plan_details?.product || "N/A"}</p>
                                      <p>Smart Resi: {zone.plan_details?.smart_resi !== undefined ? zone.plan_details.smart_resi : "N/A"}</p>
                                    </div>
                                  </div>
                                </div>
                              </Table.Cell>
