            ...
            try {
                ...
                for (const activeZone of activeZonesResponse.data) {
                    try {
                        ...
                        const zoneDetailsResp = yield axios_1.default.get(`${BRIGHTDATA_API_URL}/zone?zone=${activeZone.name}`, { headers });
                ...
                if (axios_1.default.isAxiosError(error) && error.response) {
                    ...
                    if (error.response.status === 401) {
                        throw new Error("Nieprawidłowe dane dostępowe do Bright Data API. Sprawdź BRIGHT_DATA_CUSTOMER_ID i BRIGHT_DATA_API_TOKEN.");
                    }
                    throw new Error(`Failed to fetch Bright Data zones: ${error.response.status} ${error.response.statusText} - ${JSON.stringify(error.response.data)}`);
                }
